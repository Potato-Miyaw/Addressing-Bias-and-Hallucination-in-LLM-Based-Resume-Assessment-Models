...
def render(api_base_url: str):
    ...
    with subtab1:
        ...
        **Experiment Summary:**
        ...
        with col_btn1:
            if st.button("🧪 Generate Test Cases Only", width='stretch'):
        ...
        with col_btn2:
            if st.button("🚀 Run Full Experiment", type="primary", width='stretch'):
                if not selected_models or not selected_roles:
                    st.error("Please select at least one model and one job role")
                else:
    ...
    with subtab2:
        ...
        if st.button("📄 Generate Resume", width='stretch'):
    ...
    with subtab3:
        ...
        if st.button("🔍 Analyze for Bias Markers", width='stretch'):
